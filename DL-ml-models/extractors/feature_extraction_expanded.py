...
class UpliftFeatureExtractorExpanded:
    """
    Расширенный feature extractor для uplift-моделирования
    Добавлены новые признаки по сравнению с базовой версией
    """
    ...
    def preprocess_clients(self, clients_df, train_df, treatment_df, target_df):
        ...
        # Объединение данных
        df_clients = pd.concat([train_df, treatment_df, target_df], axis=1)
        df_clients = pd.merge(df_clients, clients_df, on='client_id')
        
        # Обработка возраста
        valid_mask = (df_clients['age'] >= 15) & (df_clients['age'] <= 100)
        valid_ages = df_clients.loc[valid_mask, 'age']
        
        # Расчет квартилей для возраста
	...
	# Доля покупок в выходные (суббота=5, воскресенье=6)
        if 'transaction_weekday' in unique_trans.columns:
            weekend_days = [5, 6]
            features['weekend_purchase_ratio'] = (
                unique_trans.groupby('client_id')['transaction_weekday']
                .apply(lambda x: (x.isin(weekend_days)).sum() / len(x) if len(x) > 0 else 0)
            )

	# Доля вечерних покупок (18-23 часа)
        if 'transaction_hour' in unique_trans.columns:
    	    evening_hours = list(range(18, 24))
    	    features['evening_purchase_ratio'] = (
                unique_trans.groupby('client_id')['transaction_hour']
                .apply(lambda x: (x.isin(evening_hours)).sum() / len(x) if len(x) > 0 else 0)
    	    )

	# Концентрация покупок в топ-1 магазине
        def top_store_concentration(group):
    	    if len(group) == 0:
                return 0
    	    counts = group.value_counts()
    	    return counts.iloc[0] / len(group)

        features['store_concentration'] = client_trans['store_id'].agg(top_store_concentration)

	# Разнообразие товаров (unique_products / total_products)
        if 'unique_products_count' in features and 'total_products_purchased' in features:
	...
	# Средняя цена товара
        if 'total_trn_sum_from_iss' in features and 'total_products_purchased' in features:
    	    features['avg_price_per_product'] = self.safe_div(
                features['total_trn_sum_from_iss'],
                features['total_products_purchased'] + 1
            )

	# Среднее время между покупками (в днях)
        if 'transaction_period_days' in features and 'total_transactions' in features:
            divisor = features['total_transactions'] - 1
            divisor = divisor.replace(0, 1)
            features['avg_days_between_purchases'] = features['transaction_period_days'] / divisor

	# Доля экспресс-баллов в общих
        if 'total_express_points_received' in features and 'total_regular_points_received' in features:
	...
	# Разнообразие времени покупок (стандартное отклонение часов)
        if 'transaction_hour' in unique_trans.columns:
    	    features['purchase_time_variance'] = (
                unique_trans.groupby('client_id')['transaction_hour']
                .std().fillna(0)
            )

	# Процент потраченных баллов
        if 'total_regular_points_received' in features and 'total_regular_points_spent' in features:
    	    features['points_redemption_rate'] = self.safe_div(
                features['total_regular_points_spent'],
                features['total_regular_points_received'] + 1
            )

	# Эффективность баллов (баллы на рубль)
        if 'total_purchase_sum' in features:
	...
	# Активирован и лоялен (пока без age)
        if 'is_activated' in df.columns and 'store_loyalty_ratio' in df.columns:
            df['is_activated_and_loyal'] = ((df['is_activated'] == 1) & 
                                              (df['store_loyalty_ratio'] >= 0.7)).astype(int)
    ...
    def calculate_features(self, clients_df, train_df, treatment_df, target_df, purchases_df):
        """Основной метод"""
        # Предобработка
        processed_clients = self.preprocess_clients(clients_df, train_df, treatment_df, target_df)
        ...
        # Объединение и создание бизнес-признаков
        ...
        # Добавление treatment и target
        final_df['treatment_flg'] = processed_clients['treatment_flg']
        final_df['target'] = processed_clients['target']
        ...
        self.feature_names = [col for col in final_df.columns if col not in ['treatment_flg', 'target']]
