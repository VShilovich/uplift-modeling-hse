...
- итоговые uplift-метрики на `test_holdout`:
  - `AUUC`
  - `Qini`
  - `WAU`
  - `Uplift_at_10_perc`
...
Именно эти данные подаются в финальный `X-Learner`.
...
2. Загружает из Model Registry:
   - `model_tau_0` версии `2`
   - `model_tau_1` версии `2`
   - `model_propensity` версии `2`
...
4. Убедиться, что модели версии `2` успешно загружены;
5. Получить uplift-предсказание для mock payload.
...
Ключевая идея выбора параметров состояла не в том, чтобы сделать максимально сложный CatBoost, а в том, чтобы построить **стабильный многошаговый uplift-пайплайн**, где каждая из outcome, effect и propensity решает свою задачу с адекватным уровнем сложности.
